    ...
    <footer className="lp-footer border-t border-[#5a2a12]">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          {/* روابط المنتجات */}
          <div>
            <h4 className="font-bold text-[var(--footer-txt)] mb-4">منتجاتنا</h4>
            <ul className="flex flex-col gap-3">
              {footerLinks.products.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-[var(--footer-txt)]/90 hover:text-[var(--footer-txt)] transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          {/* روابط الدعم */}
          <div>
            <h4 className="font-bold text-[var(--footer-txt)] mb-4">المساعدة</h4>
            <ul className="flex flex-col gap-3">
              {footerLinks.support.map((link) => (
                <li key={link.label}>
                  {link.external ? (
                    <a href={link.href} className="text-[var(--footer-txt)]/90 hover:text-[var(--footer-txt)] transition-colors">
                      {link.label}
                    </a>
                  ) : (
                    <Link href={link.href} className="text-[var(--footer-txt)]/90 hover:text-[var(--footer-txt)] transition-colors">
